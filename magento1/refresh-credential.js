...
function getVaultBaseUrl() {
  const dbctlConfigFilePath = `../../../cli/db/blueprint/mysql/magento1/${ENVIRONMENT}.yml`;
...
function getCredentialEndpoint(role) {
  return `v1.1/${ORGANIZATION}/${TEAM}/db/hosted/mysql/${PRODUCT}/magento1/${ENVIRONMENT}/creds/${role}`;
}
...
async function refreshCredential(currentTimestamp) {
  ...
  const configPath = "upload/app/etc/local.xml";
