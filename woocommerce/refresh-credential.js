...
function getVaultBaseUrl() {
  const dbctlConfigFilePath = `../../../cli/db/blueprint/mysql/woocommerce/${ENVIRONMENT}.yml`;
...
function getCredentialEndpoint(role) {
  return `v1.1/${ORGANIZATION}/${TEAM}/db/hosted/mysql/${PRODUCT}/woocommerce/${ENVIRONMENT}/creds/${role}`;
}
...
async function refreshCredential(currentTimestamp) {
  ...
  const configPath = "upload/wp-config.php";
